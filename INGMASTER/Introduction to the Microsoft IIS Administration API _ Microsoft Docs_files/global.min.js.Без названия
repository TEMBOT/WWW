...
function load() {
    ...
    var args = isAzure ? { locale: msDocs.data.userLocale } : {
        ...
        headerId: isArchived ? 'MSDocsHeader-Archive' : (getMeta('uhfHeaderId') || 'MSDocsHeader-DocsL1'),
...
function setupDismissAlerts() {
    window.addEventListener('click', function (e) {
        var button = $(e.target).closest('.disclaimer > .button-dismiss')[0];
        if (button === undefined) {
            return;
        }
        var alert = button.parentNode;
        alert.classList.add("disappearing");
        setTimeout(function () { alert.parentNode.removeChild(alert); }, 500);
    }, { passive: true });
}
...
var jsllUrl = 'https://az725175.vo.msecnd.net/scripts/jsll-4.2.4.js';
var tagMapping = {
    'audience': 'aud',
    'author': 'author',
    'manager': 'manager',
...
function createSuggestedLinks() {
    ...
    fetchWithTimeout("https://docs.microsoft.com/api/search?" + toQueryString(apiConfig))
        ...
        for (var i = 0; i < SUGGESTION_LIMIT; i++) {
            links.insertAdjacentHTML('beforeend', "\n\t\t\t\t\t<li>\n\t\t\t\t\t\t<a href=\"" + suggestions[i].url + "\">" + suggestions[i].title + "</a></li>\n\t\t\t\t\t</li>\n\t\t\t\t");
        }
    });
    return links;
}
